...
import { assessWritingAction } from '../app/actions/ai';
...
const StudyHubWriting: React.FC<Props> = ({ onBack }) => {
  ...
  const handleGrade = async () => {
    ...
    const result = await assessWritingAction(text, base64Data);
    
    if (result.success) {
      setFeedback(result.text || "");
      if (result.text?.includes('9/9') || result.text?.toUpperCase().includes('EXCELLENT')) {
      ...
      <div className="w-full max-w-6xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
           ...
           <div className="bg-white border-4 border-black rounded-[2.5rem] p-8 shadow-comic overflow-y-auto max-h-[600px] relative">
              ...
              {feedback ? (
                <div className="prose prose-blue max-w-none font-bold text-gray-700 leading-relaxed">
                  <ReactMarkdown>{feedback}</ReactMarkdown>
                </div>
              ) : (
                <div className="h-full flex flex-col items-center justify-center text-center opacity-30 py-20">
                  <AlertCircle size={48} className="mb-4" />
                  <p className="font-bold">Hệ thống đang đợi bài viết của bạn!</p>
                </div>
              )}
           </div>
        </div>
