...
const BrainCandyScreen: React.FC<Props> = ({ studentName, studentId, onBack }) => {
  ...
  const generateLessonAI = async () => {
    ...
    try {
      const prompt = `Bạn là BrainCandy – một AI giáo dục có nhiệm vụ CHUẨN HÓA và CÁ NHÂN HÓA kiến thức học đường cho học sinh trung học phổ thông Việt Nam.
      Nguyên tắc cốt lõi:
      1. Kiến thức đúng chuẩn chương trình phổ thông Việt Nam hiện hành.
      2. Cung cấp kiến thức CƠ BẢN – CỐT LÕI, không lan man.
      3. Phù hợp với Lớp ${grade}, Mức độ ${level}.
      4. Trình bày ngắn gọn, dễ hiểu, ưu tiên ví dụ minh họa.
      
      Hãy tạo nội dung bài học theo mô hình 4 TẦNG cho:
      - Môn học: ${subjectName}
      - Lớp: ${grade}
      - Chủ đề: ${searchQuery || 'Kiến thức trọng tâm'}
      - Mức độ: ${level}
      - Mục tiêu học tập: Hiểu và vận dụng kiến thức cơ bản đúng chương trình.
      
      Yêu cầu đầu ra JSON:
      {
        "title": "Tên chủ đề",
        "part1_core": ["Tóm tắt 10-15 ý chính, mỗi ý không quá 2 dòng, ngôn ngữ đơn giản"],
        "part2_examples": [{"example": "Ví dụ cơ bản", "solution": "Lời giải hoặc giải thích ngắn gọn"}], // 2-3 ví dụ
        "flashcards": [{"front": "Khái niệm/Từ khóa", "back": "Giải thích ngắn gọn"}], // đúng 5 thẻ
        "quiz": [{"question": "Câu hỏi trắc nghiệm cơ bản", "options": ["A","B","C","D"], "answer": "Đáp án đúng", "explanation": "Giải thích ngắn"}] // đúng 5 câu
      }
      
      Lưu ý: Không vượt quá 500 từ cho phần kiến thức.`;
      ...
      console.error(e);
      alert("Lỗi kết nối AI. Hãy thử lại nhé!");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleQuizAnswer = (ans: string) => {
    ...
    if (ans === currentLesson?.quiz[quizIdx].answer) {
  ...
  const generatePersonalization = async () => {
    ...
    try {
      const prompt = `Bạn là BrainCandy. Hãy đánh giá kết quả học tập của học sinh ${studentName}:
      - Môn: ${currentLesson?.subject}, Lớp: ${currentLesson?.grade}
      - Chủ đề: ${currentLesson?.title}
      - Số câu đúng: ${quizScore}/5
      - Thời gian làm bài: ${timeStr}
      - Mức độ tự tin học sinh tự đánh giá: ${confidence}
      ...
      setPersonalFeedback("Kết quả của bạn rất đáng ghi nhận! Hãy tiếp tục phát huy nhé.");
      setStep('personalization');
    } finally {
      setLoading(false);
    }
      ...
      <div className="w-full max-w-5xl z-10 space-y-6">
        ...
        {step === 'setup' && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="flex bg-slate-100 p-1.5 rounded-2xl border-2 border-slate-200">
                     {['10', '11', '12'].map(g => (
                       <button 
                        ...
                        className={`flex-1 py-2 rounded-xl font-black text-lg transition-all ${grade === g ? 'bg-black text-white shadow-md' : 'text-slate-400'}`}
                       >Lớp {g}</button>
                     ))}
                  </div>
               </div>
               ...
               <div className="bg-white border-4 border-black rounded-[2.5rem] p-6 shadow-comic flex flex-col gap-3">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <Zap size={14} /> Trình độ
                  </label>
                  <div className="flex bg-slate-100 p-1.5 rounded-2xl border-2 border-slate-200">
                     {['Cơ bản', 'Trung bình', 'Khá'].map(l => (
                       <button 
                        key={l} onClick={() => setLevel(l)}
                        className={`flex-1 py-2 rounded-xl font-black text-sm md:text-base transition-all ${level === l ? 'bg-pink-500 text-white shadow-md' : 'text-slate-400'}`}
                       >{l}</button>
                     ))}
                  </div>
               </div>
               ...
               <div className="bg-white border-4 border-black rounded-[2.5rem] p-6 shadow-comic flex flex-col gap-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <Search size={14} /> Chủ đề cụ thể
                  </label>
                  <input 
                    type="text" 
                    placeholder="VD: Quang hợp, Bất đẳng thức..."
                    className="w-full bg-slate-50 border-2 border-slate-200 px-4 py-2 rounded-xl font-bold text-lg outline-none focus:border-black transition-all"
                    value={searchQuery}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6">
               {SUBJECTS.map(sub => (
                 <button
                   ...
                   className={`${sub.color} border-4 ${selectedSubject === sub.id ? 'border-pink-500 ring-4 ring-pink-100' : 'border-black'} rounded-[2.5rem] p-6 md:p-8 flex flex-col items-center gap-4 transition-all shadow-comic hover:shadow-none h-full relative group`}
                 >
                    <div className="bg-white p-4 rounded-2xl border-2 border-black shadow-sm group-hover:rotate-6 transition-transform">
                      {sub.icon}
                    </div>
            ...
            <button 
              onClick={generateLessonAI}
              disabled={loading}
              className="w-full bg-black text-white py-6 rounded-[2.5rem] font-black text-3xl shadow-comic hover:shadow-none active:scale-95 transition-all flex items-center justify-center gap-4 disabled:opacity-50"
            >
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8 max-w-4xl mx-auto w-full pb-10">
            <div className="bg-white border-4 border-black rounded-[3rem] p-8 md:p-12 shadow-comic relative overflow-hidden">
               ...
               <div className="space-y-10">
                  <section>
                    <h3 className="text-xl font-black text-pink-600 uppercase mb-6 flex items-center gap-2">
                       <div className="w-2 h-8 bg-pink-500 rounded-full"></div> PHẦN 1: KIẾN THỨC CỐT LÕI
                    </h3>
                  ...
                  <section>
                    <h3 className="text-xl font-black text-blue-600 uppercase mb-6 flex items-center gap-2">
                       <div className="w-2 h-8 bg-blue-500 rounded-full"></div> PHẦN 2: VÍ DỤ MINH HỌA
                    </h3>
        ...
        {step === 'flashcards' && currentLesson && (
          <div className="flex flex-col items-center space-y-10 w-full max-w-2xl mx-auto pb-10">
             <div className="text-center">
                <h2 className="text-3xl font-black uppercase tracking-tighter mb-2">Tóm tắt kiến thức</h2>
                <p className="text-slate-400 font-bold">Thẻ số {flashcardIdx + 1} / 5</p>
             </div>
                         ...
                         {showFlashcardAnswer ? (
                           ...
                           >
                              <p className="text-2xl md:text-3xl font-bold text-teal-600 leading-relaxed italic">
                                {currentLesson.flashcards[flashcardIdx].back}
                              </p>
                           </motion.div>
                         ) : (
                           <button 
        ...
        {step === 'quiz' && currentLesson && (
          <div className="flex flex-col items-center space-y-8 w-full max-w-3xl mx-auto pb-10">
             ...
             <div className="w-full bg-white border-4 border-black rounded-[3rem] p-10 shadow-comic min-h-[180px] flex items-center justify-center text-center">
                <h3 className="text-2xl md:text-3xl font-black text-slate-800 leading-tight">
                  {currentLesson.quiz[quizIdx].question}
                </h3>
             </div>
                ...
                {currentLesson.quiz[quizIdx].options.map((opt, i) => {
                  ...
                  return (
                    <button 
                      ...
                      className={`p-6 rounded-2xl border-4 font-black text-xl transition-all shadow-comic hover:shadow-none flex items-center justify-center text-center ${btnStyle}`}
                    >
                      {opt}
                    </button>
                  );
             ...
             <AnimatePresence>
               {showExplanation && (
                 <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full bg-blue-50 border-4 border-dashed border-blue-200 p-6 rounded-[2rem] space-y-2">
                    <p className="font-black text-blue-600 uppercase text-xs tracking-widest">Giải thích:</p>
                    <p className="font-bold text-blue-800">{currentLesson.quiz[quizIdx].explanation}</p>
                    <button 
                      onClick={nextQuiz}
                      className="mt-4 w-full bg-blue-600 text-white py-4 rounded-xl font-black shadow-comic-hover active:scale-95 transition-all"
                    >
                      {quizIdx < 4 ? 'CÂU TIẾP THEO' : 'XEM KẾT QUẢ TỔNG HỢP'}
                    </button>
                 </motion.div>
               )}
             </AnimatePresence>
          ...
          <div className="flex flex-col items-center space-y-10 w-full max-w-4xl mx-auto pb-20">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full">
                <div className="md:col-span-1 space-y-6">
                   <div className="bg-white border-4 border-black p-8 rounded-[2.5rem] shadow-comic flex flex-col items-center gap-4 text-center">
                   ...
                   <div className="bg-white border-4 border-black p-8 rounded-[2.5rem] shadow-comic flex flex-col items-center gap-2">
                      ...
                      <div className="flex gap-2 w-full">
                         {['Thấp', 'Trung bình', 'Cao'].map(c => (
                           <button 
                            key={c} onClick={() => setConfidence(c as any)}
                            className={`flex-1 py-2 rounded-xl border-2 font-black text-xs transition-all ${confidence === c ? 'bg-black text-white border-black' : 'bg-slate-50 text-slate-300 border-slate-100 hover:border-black'}`}
                           >{c}</button>
                         ))}
                      </div>
                   </div>
